...
class RealStatisticsInspection:
    ...
    async def inspect_real_statistics_page(self):
        ...
        try:
            ...
            print("📸 [DEBUG] Screenshot saved: /app/tests/statistics_page_screenshot.png")
            ...
            inspection_result = await page.evaluate("""
                () => {
                    ...
                    // Inspect all cards
                    const allCards = document.querySelectorAll('.card, .data-card, .model-card, .model-stats-card, .statistics-card, [class*="card"]');
                    ...
                    allCards.forEach((card, index) => {
                        try {
                            const cardInfo = {
                                ...
                                hasPerformanceScore: card.textContent.includes('Performance') || card.textContent.includes('Score'),
                                hasSuccessRate: card.textContent.includes('Erfolgsrate') || card.textContent.includes('Success'),
                                hasModelName: card.textContent.includes('openrouter') || card.textContent.includes('perplexity') || card.textContent.includes('abacus'),
                    ...
                    const errorElements = document.querySelectorAll('[style*="color: red"], .error, [class*="error"], [class*="fail"]');
                    ...
                    return {
                        ...
                        statisticsTabActive: document.querySelector('.nav-item[data-tab="statistics"]')?.classList.contains('active')
                    };
                }
            """)
            ...
            score_logs = [log for log in console_logs 
                         if any(keyword in log.lower() for keyword in ['score', 'success', 'performance', 'statistics', 'field-quality'])]
