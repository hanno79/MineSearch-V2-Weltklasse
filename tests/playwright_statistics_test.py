...
class StatisticsTabTester:
    ...
    async def run_comprehensive_test(self):
        ...
        print("=" * 80)
        print(f"✅ Model Cards Found: {test_results['current_behavior']['model_cards_found'] if test_results['current_behavior'] else 'N/A'}")
        print(f"✅ API Patterns: {len(test_results['api_patterns']) if test_results['api_patterns'] else 0} Kombinationen")
        print(f"✅ Splitting Tests: {len(test_results['splitting_logic']) if test_results['splitting_logic'] else 0} Test-Cases")
        print(f"✅ Rendering Logs: {test_results['rendering_logs'] if test_results['rendering_logs'] else 0} relevante Logs")
