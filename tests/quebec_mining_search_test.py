    ...
    async def execute_quebec_mining_test(self):
        ...
        try:
            ...
            for model in available_models:
                if any(keyword in model['value'].lower() for keyword in ['gpt-4o-mini',
'claude-3-haiku', 'gemini-flash']):
            ...
            start_button = await page.query_selector('button:has-text("Batch-Suche starten"),
button:has-text("Start Search"), #start-batch-search')
            if start_button:
