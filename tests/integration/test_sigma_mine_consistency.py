...
class SigmaMineConsistencyTest:
    ...
    async def run_consistency_test(self, iterations_per_model: int = 5):
        ...
        print(f"📊 {len(self.test_models)} Modelle × {iterations_per_model} Iterationen = {len(self.test_models) * iterations_per_model} Suchen")
    ...
    def calculate_statistics(self) -> Dict[str, Any]:
        ...
        for model in self.test_models:
            ...
            model_stats[model] = {
                ...
                'avg_duration': sum(r.get('duration', 0) for r in model_successful) / len(model_successful) if model_successful else 0,
                'avg_fields_found': sum(r.get('fields_found', 0) for r in model_successful) / len(model_successful) if model_successful else 0
            }
