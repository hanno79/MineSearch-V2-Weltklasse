        ...
        try:
            ...
            model_analysis = await page.evaluate("""
                () => {
                    ...
                    const modelCards = document.querySelectorAll('.mine-data-card.model-stats-card,
.model-stats-card, [data-model]');
                    ...
                    modelCards.forEach((card, index) => {
                        ...
                        if (!modelName) {
                            const modelMatch =
cardText.match(/(openrouter|perplexity|abacus|exa):[\\w-]+(?:_[^\\s]*)?/i);
                            if (modelMatch) {
                        ...
                        if (!modelName) {
                            ...
                            if (header) {
                                const headerMatch =
header.textContent.match(/(openrouter|perplexity|abacus|exa):[\\w-]+/i);
                                if (headerMatch) {
                    ...
                    console.log(`📊 [ANALYSIS] ${combinationsFound} combinations,
${individualModelsFound} individual models`);
            ...
            splitting_logs = [log for log in console_logs
                             if any(keyword in log.lower() for keyword in ['model-splitting',
