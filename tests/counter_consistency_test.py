            ...
            counters = await page.evaluate("""
                () => {
                    return {
                        ...
                        progressive_counter: document.querySelectorAll('#model-selection
input[type="checkbox"][name="model"]:checked').length,
                        ...
                        all_checkbox_counter: document.querySelectorAll('#model-selection
input[type="checkbox"]:checked').length,
            ...
            if consistency_check:
                ...
                if ui_count_match and ui_status_match:
                    ...
                    logger.warning(f"⚠️ [COUNTER-TEST] UI inconsistency:
Count='{counters['ui_selected_count']}', Status='{counters['ui_status_models']}'")
            ...
            if expected_range[0] <= main_count <= expected_range[1]:
                ...
                logger.warning(f"⚠️ [COUNTER-TEST] Unexpected model count: {main_count} (expected
{expected_range[0]}-{expected_range[1]})")
            ...
            await
page.evaluate("document.querySelectorAll('input[name=\"model\"]:checked').forEach(cb => cb.checked =
false)")
            ...
            zero_counters = await page.evaluate("""
                () => {
                    return {
                        ...
                        progressive: document.querySelectorAll('#model-selection
input[type="checkbox"][name="model"]:checked').length,
                        legacy: document.querySelectorAll('input[name="models"]:checked').length,
                        all_checkbox: document.querySelectorAll('#model-selection
input[type="checkbox"]:checked').length
                    };
                }
            """)
