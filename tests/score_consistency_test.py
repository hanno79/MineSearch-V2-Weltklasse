        ...
        try:
                ...
                () => {
                    ...
                    testScenarios.forEach(scenario => {
                        ...
                        const isConsistent = (
                            ...
                            (scenario.success_rate > 0.0 && scenario.success_rate < 1.0 && scoreResult.totalScore > 0.0 && scoreResult.totalScore < 10.0)
                        );
                        ...
                        console.log(`📊 ${scenario.name}: Success=${scenario.success_rate*100}%, Score=${scoreResult.totalScore.toFixed(1)}, Consistent=${isConsistent}`);
                    });
            ...
            if consistency_result.get('error'):
                ...
                for result in consistency_result['results']:
                    ...
                    print(f"   {status} {result['scenario']}: {result['successRate']*100}% → {result['totalScore']:.1f}/10")
                    print(f"      Components: Field={result['fieldQualityScore']:.1f}, Consistency={result['consistencyScore']:.1f}, Speed={result['speedScore']:.1f}, Cost={result['costScore']:.1f}")
        ...
        try:
                        ...
                        try {
                            ...
                            if (score !== null && successRate !== null) {
                                ...
                                if (score >= 9.0 && successRate === 0.0) {
                                    ...
                                    console.log(`⚠️ INCONSISTENCY: ${modelName} has score ${score}/10 but 0% success rate`);
                                    ...
                                    console.log(`⚠️ INCONSISTENCY: ${modelName} has 0/10 score but ${successRate}% success rate`);
                                } else {
                                    console.log(`✅ CONSISTENT: ${modelName} - Score: ${score}/10, Success: ${successRate}%`);
                                }
                            }
                        } catch (error) {
            ...
            if consistency_check['inconsistencies']:
                ...
                for issue in consistency_check['inconsistencies']:
                    print(f"   ❌ {issue['modelName']}: Score {issue['score']}/10, Success {issue['successRate']}% - {issue['issue']}")
            else:
        ...
        try:
            ...
            quality_test = await page.evaluate("""
                () => {
                    ...
                    testCases.forEach(testCase => {
                        ...
                        console.log(`📊 ${testCase.name}: Success=${testCase.success_rate*100}%, Quality=${qualityResult.score.toFixed(1)}, Level=${qualityResult.qualityLevel}`);
                    });
            ...
            if quality_test.get('error'):
                ...
                for result in quality_test['results']:
                    ...
                    print(f"   {status} {result['name']}: {result['successRate']*100}% → Quality={result['qualityScore']:.1f} ({result['qualityLevel']})")
