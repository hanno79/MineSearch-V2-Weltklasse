...
class FieldMappingDebugger:
    ...
    async def trace_complete_pipeline(self, mine_name: str, country: str, region: str):
        """Trace complete data flow from API to field assignment"""
        ...
        try:
            ...
            if raw_response:
                ...
                mining_terms = ['mine', 'mining', 'operator', 'owner', 'production', 'coordinates', 'restoration', 'costs']
            ...
            print(f"\n🧪 STEP 3: Manual Pattern Testing...")
            self.test_patterns_manually(raw_response if raw_response else "Test content: Eigentümer: TestOwner, Betreiber: TestOperator")
    ...
    def detect_mapping_issues(self, structured_data: dict) -> list:
        ...
        for field, value in structured_data.items():
            ...
            if field == 'Eigentümer' and any(term in value_str.lower() for term in ['betreiber', 'operator', 'koordinate', 'coordinate']):
                issues.append(f"{field} contains wrong data: '{value_str}'")
            elif field == 'Betreiber' and any(term in value_str.lower() for term in ['koordinate', 'coordinate', 'latitude', 'longitude']):
                issues.append(f"{field} contains coordinate data: '{value_str}'")
            elif field == 'Aktivitätsstatus' and any(term in value_str.lower() for term in ['kosten', 'costs', 'million', 'dollar']):
                issues.append(f"{field} contains cost data: '{value_str}'")
            elif 'koordinate' in field.lower() and any(term in value_str.lower() for term in ['eigentümer', 'betreiber', 'owner', 'operator']):
...
if __name__ == "__main__":
    async def main():
        ...
        print("🎯 DEBUGGING FIELD MAPPING CORRUPTION")
        print("=" * 80)
        ...
        if lac_expanse_result:
            if lac_expanse_result['raw_response_length'] == 0:
                ...
                print(f"   ➤ This explains missing data in extraction pipeline")
        ...
        print(f"\n💾 Detailed debug results saved to field_mapping_debug_results.json")
