    ...
    async def test_mine_existence(self, mine_name: str, country: str, region: str = None):
        ...
        try:
            ...
            if result.get('success'):
                ...
                # Count meaningful data_dict (not just default values)
                meaningful_fields = []
                template_indicators = []
                ...
                print(f"📊 Meaningful mining data_dict found: {len(meaningful_fields)}")
                print(f"📋 Template indicators: {len(template_indicators)}")

                if meaningful_fields:
                    print(f"✅ Real data_dict found:")
    ...
    async def main():
        ...
        if lac_expanse:
            ...
            if lac_expanse['appears_real']:
                ...
                print(f"   Found {lac_expanse['meaningful_fields']} meaningful data_dict fields")
            else:
                print(f"❌ Lac Expanse appears to be FICTIONAL or very obscure")
                print(f"   AI cannot find real mining data_dict for this location")
        ...
        if not lac_expanse or not lac_expanse['appears_real']:
            ...
            print(f"   ➤ AI cannot find real data_dict for non-existent mines")
            print(f"   ➤ System responds with template structures when no real data_dict exists")
            print(f"   ➤ Need to either:")
            print(f"     1. Test with real mines like 'Canadian Malartic'")
            print(f"     2. Improve 'no data_dict found' handling to avoid template responses")
