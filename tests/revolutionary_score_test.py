        ...
        try:
            ...
            await page.evaluate("""
                ...
                window.calculateRevolutionaryFieldQuality = function(modelData) {
                    ...
                    if (successRate === 0) return { score: 0, description: 'Feldqualität', details:
'Keine bei 0% Erfolgsrate' };
                    ...
                    if (successRate < 0.3) {
                        ...
                        console.log(`🔧 [FRESH-LOGIC] ${modelData.model_id}: Niedrige Rate
${(successRate*100).toFixed(1)}% = ${baseQuality.toFixed(1)} Punkte`);
                        return {
                    ...
                    const qualityBonus = successRate >= 0.8 ? 20 : successRate >= 0.5 ? 10 : 0; //
Bonus für hohe Erfolgsrate
                    ...
                    modelCards.forEach((card, index) => {
                        ...
                        if (scoreMatch && successRateMatch) {
                            ...
                            if (successRate === 0 && score > 0) {
                                ...
                                console.log(`🚨 [LOGIC-ERROR] ${modelName}: Score ${score}/10 with
${successRate}% success rate`);
                            }

                            // Check for other suspicious patterns
                            if (score >= 8.0 && successRate < 20.0) {
                                logicErrors.push(`${modelName}: High score ${score}/10 with low
success rate ${successRate}%`);
                                console.log(`⚠️ [SUSPICIOUS] ${modelName}: High score ${score}/10
with low success rate ${successRate}%`);
                            }
                    ...
                    const logicConsistencyPercentage = totalAnalyzed > 0 ? (logicConsistentCount /
totalAnalyzed) * 100 : 0;

                    console.log(`📊 [ANALYSIS] ${totalAnalyzed} cards analyzed, ${logicErrors.length}
logic errors found`);
            ...
            for data in score_analysis['scoreData']:
                ...
                print(f"      Score: {data['score']}/10, Success: {data['successRate']}%, Level:
{data['performanceLevel']}")
            ...
            scoring_logs = [log for log in console_logs
                           if any(keyword in log.lower() for keyword in ['revolutionary-score', '0%
erfolgsrate', 'strict rule'])]
