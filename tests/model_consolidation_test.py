        ...
        try:
            ...
            consolidation_analysis = await page.evaluate("""
                () => {
                    ...
                    console.log(`📊 [CONSOLIDATION-ANALYSIS] ${totalCards} cards, ${uniqueModels} unique models, ${duplicatesFound.length} duplicates`);
            ...
            for data in consolidation_analysis['modelData'][:10]:
                ...
                print(f"      Score: {data['score']}/10, Success: {data['successRate']}%, Searches: {data['totalSearches']}")
            
            # ANALYZE CONSOLE LOGS FOR CONSOLIDATION
            consolidation_logs = [log for log in console_logs 
                                 if any(keyword in log.lower() for keyword in ['model-consolidation', 'consolidating', 'duplicate'])]
