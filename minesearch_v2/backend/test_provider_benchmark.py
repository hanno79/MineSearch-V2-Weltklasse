...
Datum: 09.07.2025
Version: 1.0
Beschreibung: Systematischer Benchmark-Test aller Provider mit 3 kanadischen Minen
"""
...
# Provider-Gruppen für strukturierte Tests
PROVIDER_GROUPS = {
    'Web-Such-Provider': [
        ('perplexity', ['sonar', 'sonar-pro', 'sonar-deep-research', 'sonar-reasoning']),
        ('tavily', ['search', 'deep-research']),
        ('exa', ['neural-search', 'research', 'research-pro']),
...
class ProviderBenchmarkTester:
    ...
    async def test_single_model(self, provider: str, model: str, mine_data: Dict[str, str]) -> Dict[str, Any]:
    ...
    async def test_provider_group(self, group_name: str, providers: List[tuple]) -> Dict[str, Any]:
        """Teste eine Gruppe von Providern"""
        logger.info(f"\n{'#'*60}")
        logger.info(f"# Teste Provider-Gruppe: {group_name}")
        logger.info(f"{'#'*60}")
        ...
        for provider, models in providers:
            ...
            for model in models:
                ...
                for mine_data in TEST_MINES:
                    result = await self.test_single_model(provider, model, mine_data)
            ...
            group_results[provider] = provider_results
            
        return group_results
    
    async def run_complete_benchmark(self):
        """Führe kompletten Benchmark für alle Provider durch"""
        logger.info("=== STARTE KOMPLETTEN PROVIDER-BENCHMARK ===")
        logger.info(f"Test-Minen: {', '.join([m['name'] for m in TEST_MINES])}")
        logger.info(f"Anzahl Provider-Gruppen: {len(PROVIDER_GROUPS)}")
        ...
        # Teste alle Provider-Gruppen
        all_results = {}
        
        for group_name, providers in PROVIDER_GROUPS.items():
            group_results = await self.test_provider_group(group_name, providers)
        ...
        with db_manager.get_session() as session:
            ...
            logger.info(f"  - ModelStatistics: {total_tests}")
            logger.info(f"  - SearchResults: {total_results}")
            ...
            logger.info("\nTOP 5 MODELLE:")
            for model_id, success_rate, avg_fields, consistency in top_models:
                logger.info(f"  - {model_id}: {success_rate:.0%} Erfolg, "
...
async def main():
    """Hauptfunktion"""
    tester = ProviderBenchmarkTester()
    results = await tester.run_complete_benchmark()
