...
class SearchServiceAdvanced(SearchServiceOperations):
    ...
    async def smart_search(self, mine_name: str, country: Optional[str] = None,
        ...
        Returns:
            Suchergebnis der intelligenten Suche
        """
        logger.info(f"[ADVANCED] Starte Smart Search für {mine_name}")
        
        # Verwende die besten verfügbaren Modelle
        smart_models = ['perplexity:sonar-pro', 'anthropic:claude-3.7-sonnet', 'openrouter:deepseek-chat']
        available_models = list(self.registry.get_all_models().keys())
        ...
        if not selected_models:
            return {
                'success': False,
                'error': 'Keine verfügbaren Modelle für Smart Search gefunden'
            }
        
        logger.info(f"[ADVANCED] Smart Search nutzt Modelle: {selected_models}")
        
        # Führe Multi-Modell-Suche durch
        result = await self.search_with_multiple_models(
        ...
        if result.get('success'):
            result['search_strategy'] = 'smart_search'
