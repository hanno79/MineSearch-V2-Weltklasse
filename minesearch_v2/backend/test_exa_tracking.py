...
Datum: 09.07.2025
Version: 1.0
Beschreibung: Exa Provider Test mit Quellen-Tracking
"""
...
# Exa Modelle
EXA_MODELS = ['neural-search', 'research', 'research-pro']

async def test_exa():
    """Test Exa Provider mit Quellen-Tracking"""
    logger.info("=== EXA PROVIDER TEST ===")
    ...
    for model in EXA_MODELS:
        model_id = f"exa:{model}"
        logger.info(f"\n{'='*50}")
    ...
    with db_manager.get_session() as session:
        ...
        logger.info(f"\n{'='*60}")
        logger.info("EXA ZUSAMMENFASSUNG:")
        ...
        summaries = session.query(ModelSummary).filter(
            ModelSummary.model_id.like('exa:%')
        ).all()
