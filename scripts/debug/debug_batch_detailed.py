...
Datum: 11.09.2025
Version: 1.0
Beschreibung: Funktionalität für debug batch detailed
"""
...
def detailed_batch_debug():
    """detailed_batch_debug - TODO: Dokumentation hinzufügen"""
    ...
    individual_results = [{
        ...
        'data': {
            ...
            'filled_field_count': len([v for v in provider_data.values() if v and str(v).strip() and
str(v).strip() not in ['', 'None', 'null', 'nichts gefunden']])
        }
    }]
    ...
    if successful_models:
        ...
        for model_result in successful_models:
            ...
            if structured_data:
                filled_fields = len([v for v in structured_data.values() if v and str(v).strip() and
str(v).strip() not in ['-', '', 'None', 'null', 'nichts gefunden']])
    ...
    provider_filled = len([v for v in provider_data.values() if v and str(v).strip() and
str(v).strip() not in ['', 'None', 'null']])
    consolidated_filled = len([v for v in best_structured_data.values() if v and str(v).strip() and
str(v).strip() not in ['', 'None', 'null']])
