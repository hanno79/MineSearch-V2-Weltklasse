            ...
            for row in rows:
                ...
                if session_id not in sessions:
                    sessions[session_id] = {
                        ...
                        'search_duration_ms': row['search_duration'] * 1000 if row['search_duration'] else 0,  # Convert to ms
                        'success': row['success'],
    ...
    def generate_report(self, mine_name: str) -> str:
        ...
        if analysis['high_consistency_fields']:
            ...
            for field in sorted(analysis['high_consistency_fields'], key=lambda x: x['consistency_score'], reverse=True)[:5]:
                report.append(f"   📝 {field['field_name']}: {field['consistency_score']:.1%} ({field['most_common_count']}/{field['total_values']} → '{field['most_common_value']}')")
