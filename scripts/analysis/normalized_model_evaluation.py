...
class NormalizedModelEvaluationSystem:
    ...
    def calculate_model_performance(self, sessions: List[Dict]) -> Dict[str, Dict[str, Any]]:
        ...
        for model, stats in model_stats.items():
            ...
            avg_total_fields = statistics.mean(stats['fields_found_per_search']) if
stats['fields_found_per_search'] else 0
            avg_valid_fields = statistics.mean(stats['valid_fields_per_search']) if
stats['valid_fields_per_search'] else 0
...
< 0.5 and fc['total_values'] >= 3]
        ...
        template_rate = sum(stats['template_values_count'] for stats in model_performance.values())
/ sum(stats['total_searches'] for stats in model_performance.values()) if model_performance else 0
...
x['consistency_score'], reverse=True)[:5]:
                ...
                report.append(f"      🎯 Consistency: {field['consistency_score']:.1%}
({field['most_common_count']}/{field['total_values']})")
                report.append(f"      🏆 Most common: '{field['most_common_value']}'")
                report.append(f"      🤖 AI Confidence: {field['avg_confidence_score']:.2f}/1.0")
                report.append(f"      🤝 Models: {', '.join(field['models_contributing'][:3])}{'...'
if len(field['models_contributing']) > 3 else ''}")
...
x[1]['performance_score'], reverse=True)
        for i, (model, perf) in enumerate(sorted_models, 1):
            ...
            report.append(f"      📊 Avg Valid Fields: {perf['avg_valid_fields']:.1f} (Quality:
{perf['quality_rate']:.1%})")
