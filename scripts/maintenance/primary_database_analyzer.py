...
def calculate_database_score(db_info):
    ...
    if db_info['path'] == get_normalized_db_path():
...
def main():
    ...
    for db_path in candidate_dbs:
        ...
        if search_analysis:
            ...
            print(f"   🛠️  Provider: {len(search_analysis['providers'])} ({',
'.join(search_analysis['providers'][:3])})")
        ...
        if sources_analysis:
            print(f"   🌐 Sources: {sources_analysis['total_sources']} total,
{sources_analysis['unique_domains']} unique")
    ...
    for i, db in enumerate(db_analyses, 1):
        ...
        print(f"   📄 {db['path']}")
        print(f"   📊 {db['search_results']['total_results'] if db['search_results'] else 0}
Ergebnisse, {db['size_mb']:.2f} MB")
