/**
 * Advanced Search UX Test - Settings Status & Modal Fix Validation
 * Author: rahn
 * Datum: 13.08.2025
 * Beschreibung: Test der verbesserten Advanced Search UX und Settings Modal Fixes
 */

const { chromium } = require('playwright');

async function testAdvancedSearchUX() {
    console.log('🎯 [UX-TEST] Testing Advanced Search UX improvements...');
    ...
    try {
        ...
        if (quickLink) {
            ...
            if (modalVisible) {
                ...
                console.log('🔄 [TEST 4] Settings Modal Model Loading...');
                ...
                if (!hasErrorMessage && hasModels.length > 0) {
                    ...
                    if (hasModels.length > 0) {
                        // Click a model in modal
                        await hasModels[0].click();
        ...
        // Final screenshots
        await page.screenshot({ path: 'advanced_search_ux_improved.png', fullPage: true });
        
        // Test mobile view
        await page.setViewportSize({ width: 375, height: 667 });
        await page.waitForTimeout(1000);
        await page.screenshot({ path: 'advanced_search_mobile_ux.png', fullPage: true });
        
    } catch (error) {
        console.error('💥 [ERROR]', error);
        await page.screenshot({ path: 'advanced_search_ux_error.png' });
    } finally {
        await browser.close();
    }
    ...
    // Final Assessment
    console.log('');
    console.log('🏆 [ADVANCED SEARCH UX TEST RESULTS]');
    console.log('====================================');
    ...
    console.log('🏁 [ADVANCED SEARCH UX TEST] Complete');
    return successRate >= 85;
}

testAdvancedSearchUX().catch(console.error);