...
def complete_remaining_duplicates_cleanup():
    ...
    try:
            ...
            if target_result:
                ...
                for duplicate_name in duplicate_names:
                    ...
                    if duplicate_result:
                        ...
                        if ref_count > 0:
                            cursor.execute("UPDATE mine_data_fields SET company_id = ? WHERE company_id = ?", (target_id, dup_id))
        ...
        if missing_descriptions:
            ...
            type_descriptions = {
                'Exploration Property': 'Explorationsgrundstück - Gebiet mit potentiellen Mineralvorkommen in Erkundungsphase',
                'Exploration Project': 'Explorationsprojekt - Aktive Erkundungsaktivitäten zur Bewertung von Mineralvorkommen'
            }
        ...
        for type_id, name, desc in all_mine_types:
            ...
            print(f"     {desc_status} ID:{type_id:<2} - '{name}' - {desc[:50] + '...' if desc and len(desc) > 50 else desc or 'KEINE BESCHREIBUNG'}")
