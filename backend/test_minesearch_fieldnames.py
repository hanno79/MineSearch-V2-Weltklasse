...
class MineSearchFieldTest:
    ...
    async def step_5_validate_data_content(self, page):
        """Step 5: Validate actual data content in the new fields"""
        print("📊 Step 5: Validating data content...")
        
        try:
            # Look for data rows in results table
            row_selectors = [
                'tr',
                '.data-row',
                '.result-row'
            ...
            print(f"📊 Found {len(all_data)} data rows")
            
            # Analyze data for Mont Wright mine
            mont_wright_data = []
            for row in all_data:
            ...
            if mont_wright_data:
                ...
                if data_validation["production_data_found"]:
                    print("✅ Production data indicators found")
                else:
                    print("⚠️ No clear production data indicators found")
            else:
...
async def main():
    ...
    if isinstance(results['data_validation'], dict):
        ...
        print(f"   Production data found: {dv.get('production_data_found', False)}")
        print(f"   Total data rows: {dv.get('total_rows', 0)}")
