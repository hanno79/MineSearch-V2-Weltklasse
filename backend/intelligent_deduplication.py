    ...
    def _clean_companies(self, conn, dry_run=True):
        ...
        if not exact_name_duplicates.empty:
            ...
            for _, duplicate_company in exact_name_duplicates.iterrows():
                ...
                print(f"      🔄 Merge: '{duplicate_company['name']}' (ID {duplicate_company['id']})
-> (ID {original['id']})")
            ...
            for group in fuzzy_groups:
                if len(group) > 1:
                    ...
                    for _, other_company in others.iterrows():
                        print(f"         🔄 Merge: '{other_company['name']}' (ID
{other_company['id']}) -> '{best_company['name']}'")
    ...
    def _is_company_variation(self, name1, name2):
        ...
        if norm1 in norm2 or norm2 in norm1:
            ...
            suffix_words = {'ltd', 'limited', 'corp', 'corporation', 'inc', 'incorporated', 'co',
'company', 'ag', 'gmbh', 'sa', 'plc', 'llc', 'mining', 'mines'}
