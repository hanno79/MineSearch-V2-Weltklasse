...
def create_enhanced_normalized_manager():
    ...
    def _check_commodity_duplicates(self, session: Session, name: str) -> Optional[int]:
        ...
        if clean_name in translation_map:
            ...
            if translated_commodity:
                logger.debug(f"Übersetzungs-Duplikat gefunden: {name} -> {translated_commodity.name} (ID:{translated_commodity.id})")
    ...
    def _check_mine_type_duplicates(self, session: Session, name: str) -> Optional[int]:
        ...
        if clean_name in type_translation_map:
            ...
            if translated_type:
                logger.debug(f"Übersetzungs-Duplikat gefunden: {name} -> {translated_type.name} (ID:{translated_type.id})")
    ...
    def get_or_create_region(self, session: Session, name: str, country_id: int) -> Region:
        ...
        if duplicate_id:
            ...
            logger.info(f"Region-Duplikat verhindert: '{name}' -> existierende '{existing_region.name}' (ID:{duplicate_id})")
    ...
    def get_or_create_commodity(self, session: Session, name: str, symbol: str = None, unit: str = None) -> Commodity:
        ...
        if duplicate_id:
            ...
            logger.info(f"Commodity-Duplikat verhindert: '{name}' -> existierende '{existing_commodity.name}' (ID:{duplicate_id})")
    ...
    def get_or_create_mine_type(self, session: Session, name: str, description: str = None) -> MineType:
        ...
        if duplicate_id:
            ...
            logger.info(f"MineType-Duplikat verhindert: '{name}' -> existierende '{existing_type.name}' (ID:{duplicate_id})")
    ...
    def _insert_mine_data_field_3nf(self, session: Session, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"MineDataField erfolgreich eingefügt: {data.get('field_name')} (Type: {data.get('field_type')})")
            return True
...
"""
    ...
    try:
        with open("/home/hanno/projects/MineSearch/backend/minesearch/database/normalized_manager_enhanced.py", 'w') as f:
