    ...
    try:
        ...
        if not (db_path_obj.exists() and db_path_obj.is_file()):
            raise FileNotFoundError(
                f"Datenbank nicht gefunden: {db_path}. Setze ENV 'DATABASE_PATH' oder
'MINES_DB_PATH' oder konfiguriere 'DATABASE_URL' (sqlite)."
            )
    ...
    try:
        ...
        for value, field, should_be_blocked in test_cases:
            ...
            if is_blocked == should_be_blocked:
                ...
                print(f"   ❌ '{value}' in '{field}': {'BLOCKIERT' if is_blocked else 'ERLAUBT'}
(Erwartet: {'BLOCKIERT' if should_be_blocked else 'ERLAUBT'})")
