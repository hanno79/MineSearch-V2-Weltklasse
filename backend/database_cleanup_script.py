    ...
    def fix_contaminated_entry(self, entry_id: int, structured_data: dict) -> Tuple[dict, int]:
        ...
        for field_name, field_value in structured_data.items():
            if field_value and str(field_value).strip():
                ...
                if is_field_name_value(value_str, field_name):
                    logger.warning(f"[FIX] Entry {entry_id}: Feldname-Kontamination '{value_str}' in
Feld '{field_name}' → NULL")
                ...
                # 2. NULL-NORMALISIERUNG
                    ...
                    logger.debug(f"[FIX] Entry {entry_id}: NULL-Normalisierung '{value_str}' in Feld
'{field_name}' → NULL")
                    cleaned_data[field_name] = None
                    fixes_count += 1
    ...
    def generate_cleanup_report(self) -> str:
        ...
        report_lines.extend([
            ...
            f"   Backup-Größe: {os.path.getsize(self.backup_path) if
os.path.exists(self.backup_path) else '[nicht vorhanden]'} Bytes",  # REGEL 10: Keine N/A Fallbacks
            "",
            "⚠️  WICHTIG:",
