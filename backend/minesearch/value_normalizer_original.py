...
def parse_number_with_separators(value_str: str) -> Tuple[Optional[float], str, str]:
    ...
    known_units = ['ounces', 'oz', 'tonnes', 'tons', 't', 'pounds', 'lbs', 'kg', 'kilograms',
'grams', 'g', 'million', 'billion']
    ...
    logger.debug(f"[NUMBER_PARSE] Parsed '{value_str}' → Zahl:'{number_str}', Einheit:'{unit_str}',
Suffix:'{suffix_str}'")
...
class ValueNormalizer:
    ...
    def normalize_value(self, value: str, field_name: str) -> str:
        ...
        elif 'eigentümer' in field_lower or 'betreiber' in field_lower or 'owner' in field_lower or
'operator' in field_lower:
    ...
    def _normalize_region(self, value: str) -> str:
        ...
        quebec_indicators = ['quebec', 'québec', 'james bay', 'eeyou', 'nord-du-québec',
'nord-québec', 'nördliches quebec']
    ...
    def _normalize_production_amount(self, value: str) -> str:
        ...
        logger.info(f"[NORMALIZE_PRODUCTION] '{value}' → '{normalized}' (Zahl: {parsed_number},
Einheit: '{unit}' → '{normalized_unit}', Suffix: '{suffix}')")
