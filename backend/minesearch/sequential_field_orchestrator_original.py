...
class SequentialFieldOrchestrator:
    ...
    async def orchestrate_sequential_search(
        ...
        try:
            ...
            logger.info(f"[SEQUENTIAL-ORCHESTRATOR] Phase 1 abgeschlossen:
{source_discovery_result.total_sources_found} Quellen gesammelt")

            # PHASE 2: FELD-FÜR-FELD DURCHSUCHUNG
            logger.info(f"[SEQUENTIAL-ORCHESTRATOR] Phase 2: Feld-für-Feld Durchsuchung mit
{len(source_discovery_result.unique_sources)} Quellen")
            ...
            filled_fields = len([v for v in consolidated_data.values() if v and str(v).strip() and
str(v) not in ['None', 'null', '']])
            ...
            logger.info(f"[SEQUENTIAL-ORCHESTRATOR] Workflow erfolgreich abgeschlossen:
{completion_rate:.1f}% Felder gefüllt in {total_duration:.2f}s")
    ...
    async def _sequential_source_discovery(
        ...
        for idx, model_id in enumerate(models, 1):
            ...
            try:
                ...
                logger.info(f"[SOURCE-DISCOVERY-PHASE] {model_id} fand {len(model_sources)} Quellen,
{len(new_sources)} davon neu")
        ...
        logger.info(f"[SOURCE-DISCOVERY-PHASE] Abgeschlossen: {total_sources} einzigartige Quellen
in {discovery_duration:.2f}s")
    ...
    async def _field_by_field_search(
        ...
        logger.info(f"[FIELD-BY-FIELD-PHASE] Durchsuche {len(self.critical_fields)} Felder mit
{len(models)} Modellen und {len(sources)} Quellen")
                ...
                async def _run_model_search(mid=model_id, mindex=model_idx, fname=field_name):
                    async with semaphore:
                        try:
                            ...
                            logger.error(f"[FIELD-BY-FIELD-PHASE] Fehler bei {mid} für Feld
'{fname}': Timeout nach {field_search_timeout}s")
            ...
            for model_idx, model_id, result in zip(range(1, len(models) + 1), models, gathered):
                ...
                if result.success and result.value_found:
                    logger.info(f"[FIELD-BY-FIELD-PHASE] ✅ {model_id} fand für '{field_name}':
'{result.value_found}' (Confidence: {result.confidence})")
            ...
            logger.info(f"[FIELD-BY-FIELD-PHASE] Feld '{field_name}' abgeschlossen: Score
{field_report.completion_score:.2f}, Bester Wert: '{field_report.best_value}'")
    ...
    """_build_field_focused_query - TODO: Dokumentation hinzufügen"""
        self,
        field_name: str,
        mine_name: str,
        country: Optional[str],
        ...
        field_enhancements = {
            'Restaurationskosten': "Suche nach Kosten für Restauration, Sanierung, Mine Closure,
Decommissioning, Rehabilitation Costs",
        ...
        base_query += f"\n\nGib nur Informationen zu '{field_name}' zurück. Andere Felder sind nicht
relevant für diese Suche."

        return base_query

    def _create_field_completion_report(
    """_create_field_completion_report - TODO: Dokumentation hinzufügen"""
        self,
        field_name: str,
    ...
    """_consolidate_field_results - TODO: Dokumentation hinzufügen"""
        self,
        field_results: Dict[str, FieldCompletionReport]
    ) -> Dict[str, Any]:
        """
    ...
    """_update_source_database - TODO: Dokumentation hinzufügen"""
        self,
        sources: List[Dict],
        model_id: str,
        mine_name: str
    ...
    """get_workflow_statistics - TODO: Dokumentation hinzufügen"""
        self,
        result: SequentialSearchResult
    ) -> Dict[str, Any]:
        """
        ...
        stats = {
            ...
            'field_search': {
                ...
                'average_completion_score': sum(r.completion_score for r in
result.field_results.values()) / len(result.field_results) if result.field_results else 0,
                ...
                if cfg_path.exists() and cfg_path.is_file():
                    if cfg_path.suffix.lower() == '.json':
                        ...
                        try:
                            ...
                            logger.warning("[CONFIG] YAML-Unterstützung nicht verfügbar (PyYAML
fehlt). Fallback auf Default-Felder.")
                        else:
                            with cfg_path.open('r', encoding='utf-8') as f:
                        ...
                        logger.warning(f"[CONFIG] Nicht unterstütztes Dateiformat für
SEQUENTIAL_CRITICAL_FIELDS_FILE: {cfg_path.suffix}")
                else:
        ...
        if env_value:
            ...
            try:
                data_dict = json.loads(env_value)
