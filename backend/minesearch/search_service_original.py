...
class MineSearchService:
    ...
    async def search_mine(self, mine_name: str, model: str, country: Optional[str] = None,
        ...
        # cached_result = await self._check_cache(mine_name, country or "", full_model_id,
commodity=commodity, region=region)
        ...
        try:
            ...
            #     await self._cache_result(mine_name, country or "", full_model_id, result,
commodity=commodity, region=region)
    ...
    async def _search_with_provider(self, mine_name: str, country: Optional[str],
        ...
        if not provider:
            error_msg = f"Provider '{provider_name}' für Modell '{full_model_id}' ist nicht
verfügbar oder nicht korrekt konfiguriert"
            ...
            if provider_name not in available_providers:
                ...
                error_msg += ". Provider ist registriert, aber Initialisierung fehlgeschlagen
(möglicherweise fehlerhafte API-Keys)"
    ...
    async def _legacy_search(self, mine_name: str, country: Optional[str],
        ...
        provider = self.registry.get_provider(provider_name)
        if not provider:
            error_msg = f"Provider '{provider_name}' für Modell '{legacy_full_model_id}' ist nicht
verfügbar oder nicht korrekt konfiguriert"
            logger.error(f"[PROVIDER ERROR] {error_msg}")
    ...
    async def _process_search_result(self, result: SearchResult, mine_name: str,
        ...
        try:
            ...
            # with open('./normalized_debug.log', 'a') as f:
                    #                 f.write(f"DEBUG: Normalized save attempt for {mine_name} with
model {actual_model_used}\n")
            ...
            try:
                ...
                logger.error(f"[ASYNC-STATS] Failed to schedule model statistics for
{actual_model_used}: {stats_error}")
    ...
    async def _search_phase2(self, source: Dict, mine_name: str,
        ...
        provider = self.registry.get_provider(provider_name)
        if not provider:
            error_msg = f"Provider '{provider_name}' für Modell '{model}' ist nicht verfügbar oder
...
(möglicherweise fehlerhafte API-Keys)"
    ...
    """_build_phase2_query - TODO: Dokumentation hinzufügen"""
        ...
        for field, value in structured_data.items():
            ...
            if value and str(value).strip():
                ...
                if is_template_or_dummy_value(value_str, field):
                    ...
                    logger.warning(f"[ENHANCED DB QUALITY GATE] Template/Dummy-Wert abgelehnt:
{field} = '{value_str[:50]}...'")
                ...
                # PHASE 2: KRITISCHE FELDNAMEN-KONTAMINATION-DETECTION (25.08.2025)
                    ...
                    logger.error(f"[CRITICAL FIELD CONTAMINATION] Feldname als Wert blockiert:
{field} = '{value_str[:50]}...'")
                ...
                for source in sources:
                    try:
                        ...
                        if db_source:
                            ...
                            logger.debug(f"[SOURCES TRACKING] Updated {domain}:
{db_source.successful_searches}/{db_source.total_searches}")
                        else:
    ...
    """_build_search_query - TODO: Dokumentation hinzufügen"""
        ...
        4. Production data_dict (annual output, reserves)
        ...
        Provide specific, factual data_dict only. Include sources and dates.
        """

    def _validate_result(self, content: str, mine_name: str,
    """_validate_result - TODO: Dokumentation hinzufügen"""
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                for field_name, field_value in data_to_parse.items():
                    ...
                    # Handle different data_dict formats
