        ...
        for i, summary in enumerate(summaries[:5]):  # Top 5 für Radar-Chart
            radar_data.append({
                ...
                'data': [
                    ...
                    round(summary.data_success_rate * 100, 1) if hasattr(summary,
'data_success_rate') else success_rates[i]  # Daten-Erfolg
                ]
            })
...
async def get_field_comparison():
    ...
    with db_manager.get_session() as session:
        ...
        results = [{
                ...
                'avg_success_rate': float(avg_rate for field_name, avg_rate, times_found, searches, excluded, models, has_conditional in field_stats] if avg_rate else 0.0,
...
async def capture_search_statistics(stats_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"[BENCHMARK] 📊 Empfange Statistiken von {stats_data.get('model_id')} für
{stats_data.get('mine_name')}")
        ...
        with db_manager.get_session() as session:
            ...
            logger.info(f"[BENCHMARK] API-Erfolg: {api_success}, Daten-Erfolg: {data_success},
Felder: {real_fields_count}")
...
async def get_model_summaries(
    sort_by: str = Query("success_rate", description="Sortierung: success_rate, consistency, fields,
speed, total_tests"),
