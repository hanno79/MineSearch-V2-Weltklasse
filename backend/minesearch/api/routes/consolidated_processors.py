        ...
        if target_field_exists:
            ...
            if model_used in existing_models:
                ...
                if clean_value in existing_values:
                    logger.debug(f"Skipping duplicate consolidation: {original_field_name} -> {final_field_name} für Modell {model_used}")
                    return mine_data
...
def process_best_value_algorithm(mine_data, mine_name):
    ...
    for field_name in all_field_names:
        ...
        if atomic_best_value and atomic_best_value.get('confidence_score', 0) >= 50.0:
            ...
            logger.debug(f"[FALLBACK] Atomischer Wert für {mine_name}.{field_name} nicht verfügbar/unzureichend - nutze JSON-Analyse")
