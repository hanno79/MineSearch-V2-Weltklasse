...
def _process_field_value(field_value, field_name):
    ...
    if field_name in NUMERIC_FIELDS:
        ...
        if has_numbers or has_currency or has_coordinates:
            logger.info(f"[NUMERIC-PROTECTION] Numerisches Feld '{field_name}' vor
Template-Detection geschützt: '{value_str}'")
            ...
            for pattern in template_patterns:
                if re.match(pattern, value_str, re.IGNORECASE):
                    logger.info(f"[TEMPLATE-FIX] Template pattern detected in numeric field
'{field_name}': '{value_str}' -> 'Nichts gefunden'")
                    return "Nichts gefunden"
    else:
        # Nicht-numerische Felder: Normale Template-Detection
        for pattern in template_patterns:
            if re.match(pattern, value_str, re.IGNORECASE):
                logger.info(f"[TEMPLATE-FIX] Template pattern detected in field '{field_name}':
'{value_str}' -> 'Nichts gefunden'")
