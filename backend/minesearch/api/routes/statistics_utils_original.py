...
class StatisticsCalculator:
    ...
    def calculate_avg_field_coverage(self, search_results: List) -> float:
        ...
        for result in search_results:
            if result.structured_data:
                try:
                    data_dict = json.loads(result.structured_data) if isinstance(result.structured_data,
str) else result.structured_data
                    if isinstance(data, dict):
                        present_fields = sum(1 for field in important_fields if field in data_dict and data[field])
...
str) else result.structured_data
    ...
    def _assess_data_accuracy(self, data: Dict) -> float:
        ...
        if 'latitude' in data_dict and 'longitude' in data:
            try:
        ...
        if 'country' in data_dict and 'region' in data:
    ...
    def _assess_data_consistency(self, data: Dict) -> float:
        ...
        for field in numeric_fields:
            if field in data_dict and data[field]:
                try:
                    float(data[field])
                except (ValueError, TypeError):
            ...
            for i, item in enumerate(results):
                for j in range(i + 1, len(results)):
                    ...
                    if result1.structured_data and result2.structured_data:
                        try:
                            data1 = json.loads(result1.structured_data) if
isinstance(result1.structured_data, str) else result1.structured_data
                            data2 = json.loads(result2.structured_data) if
isinstance(result2.structured_data, str) else result2.structured_data
        ...
        field_consistency = {}
        for field, data_dict in field_consistency_data.items():
            if data['total'] > 0:
...
str) else result.structured_data
        ...
        return {
            'top_fields': [{'field': field, **data} for field, data_dict in top_fields],
    ...
    def compare_to_system_average(self, model_stats) -> Dict[str, Any]:
        ...
        for metric in system_averages:
            ...
            comparison[metric] = {
                ...
                'status': 'above_average' if performance_ratio > 1.05 else 'below_average' if
performance_ratio < 0.95 else 'average'
            }
        ...
        for field_name, field_data in coverage_results.items():
            if isinstance(field_data, dict):
                ...
                if coverage < 50:
                    ...
                    recommendations.append(f"Consider optimizing {field_name} data_dict collection")
