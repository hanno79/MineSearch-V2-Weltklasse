...
def _analyze_field_values(value_details, field_name=None):
    ...
    for group_value, group_details in value_groups.items():
        ...
        logger.debug(f"[ANALYZE] Gruppe '{group_value}': {total_score} Punkte ({len(group_details)}
Support, {model_bonus} Model-Bonus)")
    ...
    logger.debug(f"[ANALYZE] Feld '{field_name}' Analyse komplett: Konfidenz
{confidence_score:.1f}%, Konsistenz {consistency_score:.1f}%")
...
def _calculate_best_value(value_analysis, field_name=None):
    ...
    if actual_confidence < confidence_threshold:
        logger.debug(f"[BEST-VALUE] Konfidenz zu niedrig: {actual_confidence:.1f}% <
{confidence_threshold}% für '{field_name}'")
        return None
    ...
    if consistency < 25.0 and value_analysis.get("unique_values", 0) > 2:
        logger.warning(f"[BEST-VALUE] Niedrige Konsistenz ({consistency:.1f}%) bei '{field_name}'
mit {value_analysis.get('unique_values')} verschiedenen Werten")
        # Trotzdem den besten Wert verwenden, aber loggen

    logger.debug(f"[BEST-VALUE] Gewählt für '{field_name}': '{best_value}' (Konfidenz:
{actual_confidence:.1f}%, Support: {best_group['support_count']})")
