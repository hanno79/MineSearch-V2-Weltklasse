...
async def get_consolidated_results(
    ...
    try:
        ...
        # Process data_dict using refactored modules
            ...
            return {
                ...
                "data": {
                    "schema_comparison": {
                        ...
                        "overlap_percentage": round((len(json_fields & atomic_fields) /
max(len(json_fields), 1)) * 100, 1)
                    }
                }
            }
...
async def get_mine_statistics(mine_name: str, days_back: int = Query(30)):
    ...
    try:
        ...
        for result in results:
            try:
                import json
                data_dict = json.loads(result.structured_data) if isinstance(result.structured_data, str)
else result.structured_data
                if data:
        ...
        for field, data_dict in analyzed_fields.items():
        ...
        for model in models_used:
            ...
            if model_results:
                total_fields = sum(len(json.loads(r.structured_data) if
isinstance(r.structured_data, str) else r.structured_data if r.structured_data else {}) for r in
model_results)
                avg_response_time = sum(r.search_duration for r in model_results if
r.search_duration) / len(model_results)
                ...
                model_performance[model] = {
                    ...
                    'success_rate': round(len([r for r in model_results if r.structured_data]) /
len(model_results) * 100, 1)
                }
        ...
        return {
            ...
            'data': {
                ...
                'quality_metrics': {
                    ...
                    'consistency_rating': 'Hoch' if overall_quality > 80 else 'Mittel' if
overall_quality > 60 else 'Niedrig'
                },
                'timeline': {
