        ...
        try:
            ...
            if match:
                ...
                if len(groups) >= 3:
                    ...
                    logger.debug(f"Cross-field extraction: '{commodity_normalized}' +
'{production_formatted}' from pattern {pattern[:50]}")
                    break
...
def apply_enhanced_patterns_to_field(value: str, field: str) -> str:
    ...
    if field in ['Rohstoff', 'Rohstoffe']:
        ...
        if cross_field_data['production']:
            ...
            production_lower = cross_field_data['production'].lower()
            if any(indicator in production_lower for indicator in ['million', 'mt', 'total',
'waste', 'overburden', 'material']):
                return cross_field_data['production']
