...
class BatchResultsValidator:
    ...
    def _check_template_pattern_regression(self, results: List[Dict[str, Any]]) -> List[str]:
                    ...
                    if field_value and str(field_value).strip():
                        ...
                        if any(template in value_str for template in self.known_template_indicators):
                            ...
                            if len(value_str) > 20 and any(template in value_str for template in ['k.A.', 'LEER']):
                                issues.append(f"KRITISCH: Mögliche Template-Regression - '{value_str}' in Feld '{field_name}' sieht aus wie konvertierte echte Daten")
        ...
        if total_fields > 0:
            ...
            if template_ratio > 0.8:  # Mehr als 80% Template-Werte
                issues.append(f"KRITISCH: Template-Pattern-Regression erkannt - {template_ratio*100:.1f}% aller Felder enthalten Template-Werte")
            elif template_ratio > 0.6:  # Mehr als 60% Template-Werte
                issues.append(f"WARNUNG: Hoher Template-Anteil - {template_ratio*100:.1f}% der Felder enthalten Template-Werte")
    ...
    def _check_data_quality(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        if avg_fields_per_mine < 2:
            issues.append(f"WARNUNG: Niedrige Datenqualität - nur {avg_fields_per_mine:.1f} Felder pro Mine im Durchschnitt")
        ...
        for idx, result in enumerate(results):
            ...
            if result.get('success'):
                ...
                if not result.get('data', {}).get('structured_data'):
                    issues.append(f"KRITISCH: Mine '{mine_name}' als erfolgreich markiert aber structured_data ist leer")
                ...
                # Structured_data vorhanden aber komplett leer
                    ...
                    if len(non_empty_values) == 0:
                        issues.append(f"KRITISCH: Mine '{mine_name}' hat structured_data aber alle Werte sind leer oder Templates")
...
def log_validation_summary(validation_result: ValidationResult):
    ...
    if validation_result.is_valid:
        ...
        logger.error(f"[BATCH-VALIDATION] ❌ Batch-Ergebnisse sind INVALIDE - {validation_result.critical_count} kritische Probleme")
