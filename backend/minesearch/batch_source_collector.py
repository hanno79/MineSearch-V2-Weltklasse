...
class BatchSourceCollector:
    ...
    async def collect_sources_from_all_models(
        ...
        logger.info(f"[BATCH-SOURCE-COLLECTOR] 🚀 Starte Quellen-Sammlung für {len(models)} Modelle
über {len(mine_names)} Minen (Timeout: {timeout_seconds}s)")
    ...
    async def _collect_sources_internal(
        ...
        for batch_idx, model_batch in enumerate(model_batches):
            logger.info(f"[BATCH-SOURCE-COLLECTOR] Processing batch
{batch_idx+1}/{len(model_batches)} mit {len(model_batch)} Modellen")
        ...
        try:
            ...
            for mine_name in mine_names:
                try:
                    ...
                    logger.warning(f"[BATCH-SOURCE-COLLECTOR] ⚠️ Quellen-Sammlung für {mine_name}
mit {model_id} fehlgeschlagen: {e}")
                    continue
            ...
            logger.info(f"[BATCH-SOURCE-COLLECTOR] ✅ {model_id}: {len(unique_sources)} Quellen
gefunden, {new_sources_count} neue hinzugefügt ({duration:.2f}s)")
    ...
    async def _discover_sources_for_mine_with_model(
        ...
        try:
            ...
            logger.error(f"[BATCH-SOURCE-COLLECTOR] Source Discovery für {mine_name} mit {model_id}
fehlgeschlagen: {e}")
            return []
    ...
    def _deduplicate_sources(self, sources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"[BATCH-SOURCE-COLLECTOR] Deduplizierung: {len(sources)} →
{len(unique_sources)} einzigartige Quellen")
        ...
        try:
            with db_manager.get_session() as session:
                for source in sources:
                    ...
                    from sqlalchemy import text
                    insert_query = text("""
                        INSERT OR IGNORE INTO sources (url, domain, first_discovered_by, created_at,
reliability_score, source_type)
