...
from .models import Base, Source, SearchResult, ModelStatistics, FieldConsistency, ModelSummary,
FieldStatistics, ModelStatisticsComprehensive
...
class DatabaseManager:
    ...
    """_set_sqlite_pragma - TODO: Dokumentation hinzufügen"""
                try:
    ...
    """add_or_update_source - TODO: Dokumentation hinzufügen"""
    ...
    """save_search_result - TODO: Dokumentation hinzufügen"""
    ...
    """get_sources_for_search - TODO: Dokumentation hinzufügen"""
        ...
        with self.get_session() as session:
            ...
            for row in sessions_query.all():
                ...
                sessions_data.append({
                    ...
                    'duration_minutes': round((row.end_time - row.start_time).total_seconds() / 60,
1) if row.start_time and row.end_time else 0,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            stats['overall_success_rate'] = round((total_source_successes / total_source_searches *
100), 1) if total_source_searches > 0 else 0
    ...
    def get_model_statistics_comprehensive(self) -> List[Dict[str, Any]]:
        ...
        with self.get_session() as session:
            ...
            models_data = [{
                    ...
                    'success_rate': round(stat.success_rate_percent, 1 for stat in comprehensive_stats],
    ...
    def update_model_statistics_comprehensive(self, model_id: str):
        ...
        with self.get_session() as session:
            try:
                ...
                # Calculate data_dict quality metrics
                ...
                for result in search_results:
                    if result.structured_data:
                        try:
                            import json
                            data_dict = json.loads(result.structured_data) if
isinstance(result.structured_data, str) else result.structured_data
                            if isinstance(data, dict):
                                # Count filled fields (exclude N/A, empty strings, null values)
                                na_values = ['N/A', '', None, 'null', 'Unknown', 'Keine Angabe']
                                filled_count = len([v for v in data.values() if v and str(v) not in
na_values and not str(v).startswith('_')])
                ...
                logger.info(f"[STATS-CALC] Model {model_id}: {total_filled_fields} total fields /
{valid_data_searches} searches = {actual_avg_fields_found:.2f} avg_fields_found")
                ...
                field_completeness = (actual_avg_fields_found / total_expected_fields) * 100 if
total_expected_fields > 0 else 0
                ...
                speed_score = max(0, 100 - (avg_response_time * 2)) if avg_response_time > 0 else
100  # Penalty for slow responses
    ...
    def get_recent_search_results(self, mine_name: str, hours_back: int = 24, limit: int = 10,
    """get_recent_search_results - TODO: Dokumentation hinzufügen"""
session_id: Optional[str] = None) -> List[SearchResult]:
        ...
        with self.get_session() as session:
            ...
            logger.info(f"[SESSION-ISOLATION] Found {len(results)} results for mine='{mine_name}',
