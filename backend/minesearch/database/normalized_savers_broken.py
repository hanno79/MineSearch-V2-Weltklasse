    ...
    if db_session is not None:
        ...
        for field_name, field_value in structured_data.items():
            ...
            if sources and len(sources) > 0:
                source_name = sources[0].get('source_name', 'Unknown')
            ...
            if field_type == 'normalized':
                ...
                if 'rohstoffabbau' in field_name.lower() or 'commodity' in field_name.lower():
                    commodity_id = manager_self.get_or_create_commodity(atomic_value, smart_extractor=manager_self.smart_extractor, db_session=session)
                ...
                # Firmen-Normalisierung  
                    ...
                    company_id = self.get_or_create_company(atomic_value, smart_extractor=self.smart_extractor, db_session=session)
                ...
                # Status-Normalisierung
                    ...
                    activity_status_id = self.get_or_create_activity_status(atomic_value, smart_extractor=self.smart_extractor, db_session=session)
                ...
                # Minentyp-Normalisierung
                    ...
                    mine_type_id = self.get_or_create_mine_type(atomic_value, smart_extractor=self.smart_extractor, db_session=session)
                ...
                # Region-Normalisierung
                    ...
                    region_id = self.get_or_create_region(atomic_value, smart_extractor=self.smart_extractor, country_id=country_for_region, db_session=session)
    ...
    with self.get_session() as local_session:
        for field_name, field_value in structured_data.items():
            ...
            if field_type == 'normalized':
                ...
                # Rohstoff-Normalisierung
                if 'rohstoffabbau' in field_name.lower() or 'commodity' in field_name.lower():
                    commodity_id = self.get_or_create_commodity(atomic_value, smart_extractor=self.smart_extractor, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Commodity '{atomic_value}' -> ID {commodity_id}")
                
                # Firmen-Normalisierung  
                elif 'eigentümer' in field_name.lower() or 'betreiber' in field_name.lower() or 'owner' in field_name.lower() or 'operator' in field_name.lower():
                    company_type = 'owner' if 'eigentümer' in field_name.lower() or 'owner' in field_name.lower() else 'operator'
                    company_id = self.get_or_create_company(atomic_value, smart_extractor=self.smart_extractor, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Company '{atomic_value}' ({company_type}) -> ID {company_id}")
                
                # Status-Normalisierung
                elif 'aktivitätsstatus' in field_name.lower() or 'activity' in field_name.lower() or 'status' in field_name.lower():
                    activity_status_id = self.get_or_create_activity_status(atomic_value, smart_extractor=self.smart_extractor, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Status '{atomic_value}' -> ID {activity_status_id}")
                
                # Minentyp-Normalisierung
                elif 'minentyp' in field_name.lower() or 'mine_type' in field_name.lower() or 'type' in field_name.lower():
                    mine_type_id = self.get_or_create_mine_type(atomic_value, smart_extractor=self.smart_extractor, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Mine Type '{atomic_value}' -> ID {mine_type_id}")
                ...
                # Region-Normalisierung
                    ...
                    region_id = self.get_or_create_region(atomic_value, smart_extractor=self.smart_extractor, country_id=country_for_region, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Region '{atomic_value}' -> ID {region_id}")
...
def save_search_result_normalized(
    ...
    Integriert Smart Value Extractor + Template Detection + Confidence Scoring
    """
    try:
        ...
        mine_id = self.get_or_create_mine(mine_name, smart_extractor=self.smart_extractor, structured_data, db_session=db_session)
        ...
        if db_session is not None:
            ...
            save_mine_field_data(self, mine_id, search_result_id, structured_data, model_used, sources, session_id=session_id, db_session=session)
            ...
            with self.get_session() as session_local:
                ...
                save_mine_field_data(self, mine_id, search_result_id, structured_data, model_used, sources, session_id=session_id)
...
def insert_mine_data_field_3nf(
    ...
    Trennt NORMALIZED (Reference-IDs) von PRIMITIVE (Wert-Daten) Feldern
    """
    try:
        ...
        # 3. INSERT mit SQLAlchemy Core (für bessere NULL-Handling)
        insert_sql = text("""
