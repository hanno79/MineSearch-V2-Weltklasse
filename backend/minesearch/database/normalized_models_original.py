...
UniqueConstraint, Index, Numeric, Date, JSON
...
class MineCommodity(NormalizedBase):
    ...
    """to_dict - TODO: Dokumentation hinzufügen"""
        return {
            ...
            'production_volume_per_year': float(self.production_volume_per_year) if
self.production_volume_per_year is not None else None,
            'unit': self.unit,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None
        }
...
class FieldValue(NormalizedBase):
    ...
    id = Column(Integer, primary_key=True)
    search_session_id = Column(Integer, ForeignKey('search_sessions.id', ondelete='CASCADE'),
...
class FieldValueSource(NormalizedBase):
    ...
    """to_dict - TODO: Dokumentation hinzufügen"""
        return {
            ...
            'source_id': self.source_id,
            'extraction_confidence': float(self.extraction_confidence) if self.extraction_confidence
is not None else 50.0,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None
        }
...
class Source(NormalizedBase):
    ...
    source_type = Column(String(50), nullable=False, default='unknown')  # government, exchange,
industry, database, document
