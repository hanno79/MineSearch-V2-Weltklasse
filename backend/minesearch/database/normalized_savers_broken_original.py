...
field_name.lower() else 'operator'
                    company_id = self.get_or_create_company(atomic_value,
smart_extractor=self.smart_extractor, db_session=session)
...
'status' in field_name.lower():
                    activity_status_id = self.get_or_create_activity_status(atomic_value,
smart_extractor=self.smart_extractor, db_session=session)
...
in field_name.lower():
                    mine_type_id = self.get_or_create_mine_type(atomic_value,
smart_extractor=self.smart_extractor, db_session=session)
                ...
                # Region-Normalisierung
                    ...
                    region_id = self.get_or_create_region(atomic_value,
smart_extractor=self.smart_extractor, country_id=country_for_region, db_session=session)
...
'Quellenangaben', 'sources', 'references']:
            ...
            if field_type == 'normalized':
                ...
                # Rohstoff-Normalisierung
                if 'rohstoffabbau' in field_name.lower() or 'commodity' in field_name.lower():
                    commodity_id = self.get_or_create_commodity(atomic_value,
smart_extractor=self.smart_extractor, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Commodity '{atomic_value}' -> ID {commodity_id}")
...
smart_extractor=self.smart_extractor, db_session=local_session)
                ...
                # Region-Normalisierung
                    ...
                    region_id = self.get_or_create_region(atomic_value,
smart_extractor=self.smart_extractor, country_id=country_for_region, db_session=local_session)
                    logger.info(f"[NORMALIZATION] Region '{atomic_value}' -> ID {region_id}")
...
def save_search_result_normalized(
    ...
    try:
        ...
        mine_id = self.get_or_create_mine(mine_name, smart_extractor=self.smart_extractor,
structured_data, db_session=db_session)
        ...
        if db_session is not None:
            ...
            save_mine_field_data(self, mine_id, search_result_id, structured_data, model_used,
sources, session_id=session_id, db_session=session)
            ...
            with self.get_session() as session_local:
                ...
                logger.info(f"[FIELD_SAVE_DEBUG] About to call save_mine_field_data (else branch)
with {len(structured_data)} fields")
                save_mine_field_data(self, mine_id, search_result_id, structured_data, model_used,
sources, session_id=session_id)
                logger.info(f"[FIELD_SAVE_DEBUG] Finished save_mine_field_data call (else branch)")
                # ENTFERNT 04.09.2025: Koordinaten werden nicht mehr in mines Tabelle gespeichert

        logger.info(f"✅ NORMALIZED SAVE: Mine='{mine_name}' Model='{model_used}'
Fields={fields_found} Quality={data_quality_score:.2f}")
