...
class MultiModelSearchOrchestrator:
    ...
    """__init__ - TODO: Dokumentation hinzufügen"""
        ...
        logger.info(f"[ORCHESTRATOR-INIT] Provider Registry Status:
{len(provider_registry._providers)} Provider geladen")
        if not provider_registry._providers:
            logger.info("[ORCHESTRATOR-INIT] Provider Registry wird initialisiert (idempotent)...")
        else:
            logger.info("[ORCHESTRATOR-INIT] Registry bereits gefüllt – führe idempotente
Initialisierung aus (wird ggf. übersprungen)")
    ...
    def _validate_provider_response(self, search_result, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_report['has_content']:
                ...
                logger.info(f"[VALIDATION-CONTENT] {model_id}: Content length
{validation_report['content_length']} chars")
            ...
            if structured_data and isinstance(structured_data, dict):
                ...
                for field, value in structured_data.items():
                    if value and str(value).strip() and str(value).strip() not in ['nichts
gefunden', '', 'None', 'null', '-']:
                ...
                logger.info(f"[VALIDATION-STRUCTURED] {model_id}:
{validation_report['filled_field_count']}/{validation_report['field_count']} fields with data")
            ...
            if validation_report['is_valid']:
                logger.info(f"[VALIDATION-SUCCESS] {model_id}: Response is valid with
{validation_report['filled_field_count']} data_dict fields")
            else:
                validation_report['issues'].append("No useful data_dict found")
    ...
    async def orchestrate_multi_model_search(
        ...
        logger.info(f"[ORCHESTRATOR] Starte Multi-Model-Suche für '{mine_name}' mit {len(models)}
Modellen (session_id: {session_id})")
        ...
        if use_all_db_sources and db_sources:
            ...
            logger.info(f"[ORCHESTRATOR] Top Domains: {dict(sorted(domain_count.items(), key=lambda
x: x[1], reverse=True)[:5])}")
            ...
            try:
                ...
                logger.info(f"[ORCHESTRATOR] Source Discovery completed: {len(shared_sources)}
sources in {discovery_duration:.2f}s")
        ...
        for batch_idx, model_batch in enumerate(model_batches):
            logger.info(f"[ORCHESTRATOR] Processing batch {batch_idx+1}/{len(model_batches)} with
{len(model_batch)} models")
...
and str(v).strip() != 'nichts gefunden'])
                    logger.info(f"[ORCHESTRATOR-DB-FIELDS] Saving {result.model_id}:
{filled_fields}/{data_fields} fields with data")
                    ...
                    logger.error(f"[ORCHESTRATOR-DB-ERROR] Model {result.model_id} has EMPTY
structured_data - cannot save!")
                ...
                with legacy_db_manager.get_session() as tx_session:
                    try:
                        ...
                        tx_session.commit()
                        logger.info(f"[ORCHESTRATOR-DB-SUCCESS] ✅ Atomic save committed for
{result.model_id} (normalized_id={normalized_result_id})")
                        logger.info(f"[ORCHESTRATOR-STATS] Statistics auto-updated via
save_search_result for {result.model_id}")
        ...
        logger.info(f"[ORCHESTRATOR] Orchestration completed:
{len(successful_results)}/{len(models)} models successful in {total_duration:.2f}s")
        ...
        try:
            ...
            if hasattr(search_result, 'error') and search_result.error:
                ...
                if 'pay-as-you-go limit' in error_msg or 'quota' in error_msg or 'budget' in error_msg:
                    ...
                    logger.warning(f"[ORCHESTRATOR-API-404] {model_id}: API-Endpoint nicht
erreichbar - {search_result.error}")
                elif 'unauthorized' in error_msg or '401' in error_msg or 'invalid key' in error_msg:
                    logger.warning(f"[ORCHESTRATOR-API-AUTH] {model_id}: Authentifizierung
fehlgeschlagen - {search_result.error}")
                else:
            ...
            logger.info(f"[ORCHESTRATOR-VALIDATION] {model_id} validation complete:
valid={validation_report['is_valid']}")
            ...
            if search_result.success:
                ...
                if structured_data:
                    filled_fields = len([v for v in structured_data.values() if v and str(v).strip()
and str(v).strip() != 'nichts gefunden'])
                    logger.info(f"[ORCHESTRATOR-FILLED] Model {model_id} found data_dict in
{filled_fields}/{len(structured_data)} fields")
                    ...
                    for field in critical_fields:
                        if field in structured_data:
                            ...
                            logger.warning(f"[ORCHESTRATOR-MISSING] Model {model_id} - Field
'{field}' not found in structured_data")
                ...
                provider_name = search_result.metadata.get("provider", 'unknown') if
hasattr(search_result, 'metadata') and search_result.metadata else 'unknown'
        ...
        try:
            ...
            with db_manager.get_session() as db_session:
                ...
                for db_result in db_results:
                    ...
                    if structured_data:
                        filled_fields = len([v for v in structured_data.values() if v and
str(v).strip() and str(v).strip() != 'nichts gefunden'])
                        if filled_fields > 0:
                            ...
                            logger.warning(f"[DB-CONSISTENCY] Model {db_result.model_used} has empty
structured_data in DB")
                    else:
                        consistency_report['empty_results'] += 1
                ...
                logger.info(f"[DB-CONSISTENCY] Summary - Data:
{consistency_report['models_with_data']}, Empty: {consistency_report['empty_results']}")
