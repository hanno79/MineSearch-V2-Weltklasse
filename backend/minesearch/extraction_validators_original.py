...
def validate_coordinate(value: str, coord_type: str) -> Optional[str]:
    ...
    if any(word in str(value).lower() for word in ['jahr', 'year', 'datum', 'date', 'aktiv',
'active', 'geschlossen', 'closed']):
    ...
    try:
        ...
        if '.' in str(value):
            ...
            if decimal_places < 2:  # Mindestens 2 Nachkommastellen
                logger.warning(f"[COORD-VALIDATION] Koordinate hat zu wenig Präzision
({decimal_places} Nachkommastellen): {value}")
                return None
        else:
        ...
        if coord_type == 'x':  # Latitude
            ...
            if original_had_sign:
                ...
                if corrected_coord != coord_float:
                    logger.info(f"[COORD-FALLBACK] Longitude ohne Vorzeichen korrigiert:
{coord_float} → {corrected_coord} (Land: {country_context})")
...
def _auto_correct_longitude_sign(longitude: float, country: Optional[str]) -> float:
    ...
    elif country_lower in ['brazil', 'brasilien', 'argentina', 'argentinien', 'chile', 'peru',
'colombia', 'kolumbien', 'venezuela', 'bolivia', 'bolivien', 'ecuador', 'uruguay', 'paraguay']:
    ...
    elif country_lower in ['germany', 'deutschland', 'france', 'frankreich', 'spain', 'spanien',
'italy', 'italien', 'poland', 'polen', 'sweden', 'schweden', 'norway', 'norwegen', 'finland',
'finnland']:
    ...
    elif country_lower in ['china', 'india', 'indien', 'japan', 'south korea', 'südkorea',
'indonesia', 'indonesien', 'thailand', 'philippines', 'philippinen']:
    ...
    elif country_lower in ['morocco', 'marokko', 'mauritania', 'mauretanien', 'senegal', 'gambia',
'guinea-bissau', 'guinea', 'sierra leone', 'liberia', 'ivory coast', 'elfenbeinküste', 'ghana',
'togo', 'benin', 'nigeria', 'cameroon', 'kamerun']:
