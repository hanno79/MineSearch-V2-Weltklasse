            ...
            async with httpx.AsyncClient(timeout=model_config.timeout) as client:
                response = await client.post(
                    ...
                    json={
                        ...
                        "max_tokens": min(model_config.max_tokens, 4096) if model_config.id ==
'gpt-4-turbo' else model_config.max_tokens
                    }
                )
                ...
                if response.status_code != 200:
                    ...
                    if model_config:
                        ...
                        return await self._search_standard(enhanced_query, model_config, options,
mine_name, country, start_time)
                    else:
                        return SearchResult(
                            success=False,
                            content="",
