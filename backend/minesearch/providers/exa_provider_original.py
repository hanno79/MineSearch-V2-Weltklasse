...
class ExaProvider(AbstractProvider):
    ...
    async def search(self, query: str, model_id: str, options: Dict[str, Any]) -> SearchResult:
        ...
        if not discovered_sources and not skip_discovery and mine_name:
            ...
            if use_all_sources:
                logger.info(f"[EXA] 🔥 2-PHASEN WORKFLOW: Nutze ALLE {len(discovered_sources)}
...
self.data_extractor.extract_structured_data_with_sources(ai_response, mine_name, country)
    ...
    """_build_semantic_query - TODO: Dokumentation hinzufügen"""
        ...
        if model_id in ['neural-search', 'research', 'research-pro']:
            ...
            query_parts = [
                ...
                f"including specific restoration costs, closure costs, environmental liabilities
