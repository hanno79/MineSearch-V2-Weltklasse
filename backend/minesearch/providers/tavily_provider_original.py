...
class TavilyProvider(AbstractProvider):
    ...
    async def search(self, query: str, model_id: str, options: Dict[str, Any]) -> SearchResult:
        ...
        if not discovered_sources and not skip_discovery and mine_name:
            ...
            if use_all_sources:
                logger.info(f"[TAVILY] 🔥 2-PHASEN WORKFLOW: Nutze ALLE {len(discovered_sources)}
übergebenen DB-Quellen ohne Filter")
            else:
        ...
        try:
            async with httpx.AsyncClient(timeout=model_config.timeout) as client:
                ...
                if result.get('results'):
                    ...
                    logger.debug(f"[TAVILY] Erstes Ergebnis: Title='{first_result.get('title')}',
URL='{first_result.get('url')}'")
                ...
                extracted_data =
self.data_extractor.extract_structured_data_with_sources(ai_response, mine_name, country)
    ...
    """_build_mining_query - TODO: Dokumentation hinzufügen"""
    ...
    """_generate_query_variants - TODO: Dokumentation hinzufügen"""
...
{len(tavily_response.get("results", []))} Quellen:**\n")
...
- Cross-Reference verschiedene Informationsquellen"""
    ...
    """_build_mining_query_with_sources - TODO: Dokumentation hinzufügen"""
        self,
        base_query: str,
        mine_name: str,
        country: str,
    ...
    def _apply_quality_gates(self, extracted_data: Dict[str, Any], mine_name: str) -> Dict[str, Any]:
        ...
        data_dict = extracted_data.get("data", {})

        # Verhindere "Koordinaten" als Betreiber (wie OpenRouter)
        if data.get('Betreiber'):
            betreiber = str(data['Betreiber']).strip()
            if betreiber.lower() in ['koordinaten', 'coordinates', 'coords', 'koordinate'] or
'koordinaten:' in betreiber.lower():
