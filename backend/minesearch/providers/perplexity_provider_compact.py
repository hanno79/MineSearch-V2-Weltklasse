"""
Compact Perplexity Provider
Kompakte Version des Perplexity Providers
...
class PerplexityProvider(AbstractProvider):
    """Perplexity Provider für Mining-Suchen"""

    def __init__(self, api_key: str = None, config: Dict[str, Any] = None):
        """Initialisiere Perplexity Provider"""
        super().__init__(api_key, config)
        self.name = "perplexity"
        self.base_url = config.get("base_url", 'https://api.perplexity.ai') if config else 'https://api.perplexity.ai'
    ...
    async def search(self, query: str, model: str = None, **kwargs) -> List[SearchResult]:
        ...
        """
        try:
            logger.info(f"[PERPLEXITY] Starte Suche für: {query}")
            ...
            for term in search_terms:
                try:
                    search_results = await self._search_with_perplexity(term, model)
            ...
            logger.info(f"[PERPLEXITY] ✅ {len(unique_results)} Ergebnisse gefunden")
            return unique_results
            
        except Exception as e:
            logger.error(f"[PERPLEXITY] ❌ Fehler bei Suche: {e}")
            return []
    ...
    async def search_url(self, url: str, **kwargs) -> Optional[SearchResult]:
        ...
        """
        try:
            logger.info(f"[PERPLEXITY] Suche URL: {url}")
            
            # Simuliere URL-Suche
            result = await self._search_url_with_perplexity(url)
            
            if result:
                logger.info(f"[PERPLEXITY] ✅ URL erfolgreich gesucht")
                return result
            else:
                logger.warning(f"[PERPLEXITY] Keine Ergebnisse für URL: {url}")
                return None
                
        except Exception as e:
            logger.error(f"[PERPLEXITY] ❌ Fehler beim Suchen der URL {url}: {e}")
            return None
    ...
    async def _search_with_perplexity(self, query: str, model: str = None) -> List[SearchResult]:
        """Führe Suche mit Perplexity API durch"""
        try:
            # Simuliere Perplexity API-Aufruf
            ...
            for i in range(3):
                result = SearchResult(
                    url=f"https://example.com/perplexity/search?q={query}&result={i}",
                    title=f"Perplexity search result {i+1} for {query}",
                    content=f"This is search result {i+1} for the query '{query}' using Perplexity search.",
            ...
            logger.error(f"Fehler bei Perplexity-Suche: {e}")
            return []

    async def _search_url_with_perplexity(self, url: str) -> Optional[SearchResult]:
        """Suche spezifische URL mit Perplexity"""
        try:
            # Simuliere URL-Suche
            result = SearchResult(
                url=url,
                title=f"Content from {url}",
                content=f"This is content from {url} found using Perplexity search.",
                source_type='perplexity_url_search',
                relevance_score=0.8,
                metadata={
                    'provider': 'perplexity',
                    'url': url,
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        return {
            'name': 'perplexity',
            'description': 'Perplexity für Mining-Suchen',
            'capabilities': [
                'web_search',
                'url_search',
                'ai_enhanced_results',
    ...
    async def _make_api_request(self, endpoint: str, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Mache API-Anfrage an Perplexity"""
        try:
                    ...
                    'results': [
                        {
                            ...
                            'content': 'This is example content from Perplexity search.',
                            'score': 0.9
                        }
                    ]
                }
