...
region=None, commodity=commodity)
    ...
    async def _scrape_url(self, client: httpx.AsyncClient, url: str,
        ...
        if model_id == 'ai-extract':
            ...
            params['instruction'] = f"""Extract structured mining data_dict for the mine
"{options.get("mine_name", '')}" from this webpage.
...
PDF müsste mit spezialisierten Tools extrahiert werden.",
                        'title': 'PDF Document',
                        'is_pdf': True,
    ...
    async def scrape_sources(self, discovered_sources: List[Dict], model_id: str, options: Dict[str,
Any]) -> List[Dict[str, Any]]:
        ...
        for source in discovered_sources[:10]:
            ...
            result = await self.search(
                query=f"Extract mining data_dict from: {url}",
                model_id=model_id,
...
Analysiere die folgenden ScrapingBee Web-Scraping Ergebnisse und extrahiere strukturierte
...
"""
            ...
            user_prompt = f"""Analysiere die ScrapingBee Scraping-Ergebnisse und extrahiere
