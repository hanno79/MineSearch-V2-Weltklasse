...
try:
    ...
    raise ImportError("OpenRouterProvider: Benötigtes Modul 'minesearch.specialized_prompts' konnte
nicht importiert werden. Bitte installieren Sie die Abhängigkeit oder prüfen Sie den PYTHONPATH.")
from exc
try:
    from minesearch.specialized_prompts_impl import SpecializedPrompts as ImplSpecializedPrompts
except ImportError as exc:
    raise ImportError("OpenRouterProvider: Benötigtes Modul 'minesearch.specialized_prompts_impl'
konnte nicht importiert werden. Wenn dieses Modul optional ist, entfernen Sie Abhängigkeiten oder
stellen Sie die Implementierung bereit.") from exc
    ...
    async def search(self, query: str, model_id: str, options: Dict[str, Any]) -> SearchResult:
        ...
        try:
            ...
            if not discovered_sources and not skip_discovery and mine_name:
                ...
                if use_all_sources:
                    logger.info(f"[OPENROUTER] 🔥 2-PHASEN WORKFLOW: Nutze ALLE
{len(discovered_sources)} übergebenen DB-Quellen ohne Filter")
            ...
            async with httpx.AsyncClient(timeout=model_config.timeout) as client:
                ...
                if extracted_data['data'].get('Betreiber'):
                    ...
                    if betreiber.lower() in ['koordinaten', 'coordinates', 'coords', 'koordinate']
or 'koordinaten:' in betreiber.lower():
                ...
                if extracted_data['data'].get('Restaurationskosten'):
                    ...
                    if resto in suspicious_values or (isinstance(resto, str) and any(sv in resto for
sv in suspicious_values)):
    ...
    """_enhance_query_for_no_web - TODO: Dokumentation hinzufügen"""
        ...
        if discovered_sources:
            ...
            sources_text += "Auch wenn du nicht direkt darauf zugreifen kannst, kennst du
möglicherweise Daten aus diesen Quellen:\n\n"
            ...
            other_sources = [s for s in discovered_sources if s.get('type') not in ['government',
'database', 'exchange']]
    ...
    def _handle_api_error(self, response: httpx.Response) -> str:
        ...
        if response.status_code == 401:
            ...
            try:
                ...
                logger.warning(f"[OPENROUTER] Fehlerhafte JSON-Antwort bei Status
...
- x-Koordinate: [GPS Latitude mit min. 4 Nachkommastellen oder leer]
