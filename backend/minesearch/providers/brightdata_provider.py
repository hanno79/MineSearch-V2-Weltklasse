...
class BrightdataProvider(AbstractProvider):
    ...
    async def search(self, query: str, model_id: str, options: Dict[str, Any]) -> SearchResult:
        ...
        try:
            ...
            if not discovered_sources and not skip_discovery and mine_name:
                ...
                if use_all_sources:
                    logger.info(f"[BRIGHTDATA] 🔥 2-PHASEN WORKFLOW: Nutze ALLE {len(discovered_sources)} übergebenen DB-Quellen ohne Filter")
                else:
    ...
    async def _web_scraper_search(self, query: str, options: Dict[str, Any]) -> SearchResult:
        ...
        for result in scrape_results:
            ...
            logger.debug(f"[BrightData-WebScraper] Processing result: success={result.get('success')}, html_length={html_length}")
            if result['success'] and result['html']:
                ...
                if extracted is None:
                    extracted = {}
                logger.debug(f"[BrightData-WebScraper] Extracted data fields: {list(extracted.keys()) if extracted else 'None'}")
                if extracted:
                ...
                try:
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            logger.info(f"[BrightData-SERP] Response status: {response.status}, Content length: {html_len}")
    ...
    def _aggregate_scraped_data(self, scraped_data: List[Dict], mine_name: str, 
        ...
        try:
            ...
            if not aggregated or aggregated_len == 0:
                logger.warning(f"[BrightData] Keine Daten extrahiert für {mine_name} - Suche als fehlgeschlagen markieren (REGEL 10: Keine Dummy-Werte)")
            ...
            logger.info(f"[BrightData] Aggregierte Daten für {mine_name}: {aggregated_field_count} Felder, Qualität: {aggregated.get('data_quality_score', 0):.2f}")
            return aggregated
...
Analysiere die folgenden BrightData Web-Scraping Ergebnisse und extrahiere strukturierte Mining-Daten für die Mine "{mine_name}" in {country}.
...
"""
            ...
            user_prompt = f"""Analysiere die BrightData Scraping-Ergebnisse und extrahiere Mining-Daten für "{mine_name}" in {country}.
