...
try:
    response = requests.get("http://localhost:8000/api/consolidated/results?exclude_exa=true&days_back=30&sort_by=mine_name&order=asc")
    data = response.json()
    ...
    if data['success'] and data['data']['consolidated_results']:
        ...
        for i, mine in enumerate(data['data']['consolidated_results']):
            ...
            for field, value in best_values.items():
                ...
                field_names = ['Betreiber', 'Eigentümer', 'x-Koordinate', 'y-Koordinate', 'Region', 'Restaurationskosten']
        ...
        for mine in data['data']['consolidated_results'][:5]:
            ...
            for field_name, field_data in detailed.items():
                ...
                if confidence >= 90 and display_value != mine['mine_name']:  # Nicht Mine-Name
                    high_confidence_examples.append(f'{mine["mine_name"][:20]}... -> {field_name}: {confidence}% ("{display_value}")')
